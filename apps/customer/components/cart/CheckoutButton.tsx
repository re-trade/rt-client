...
interface CheckoutButtonProps {
  ...
  checkoutStep: 'initial' | 'creating' | 'processing' | 'redirecting' | 'payment_failed';
  onCheckout: () => void;
  onRetryPayment: () => void;
  onForceReset: () => void;
...
export default function CheckoutButton({
  ...
  const getButtonText = () => {
    if (checkoutStep !== 'initial' && checkoutStep !== 'payment_failed' && !isProcessing) {
      return 'Đặt lại thanh toán';
    }
    ...
    switch (checkoutStep) {
      ...
      case 'processing':
        return 'Đang tiến hành thanh toán';
      case 'redirecting':
        return 'Đang chuyển hướng...';
      case 'payment_failed':
        return 'Khởi tạo lại thanh toán';
      default:
  ...
  const getButtonClickHandler = () => {
    ...
    if (checkoutStep !== 'initial' && !isProcessing) {
      return onForceReset;
    }
    ...
    <div className="rounded-xl border border-orange-100 bg-white shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
      <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 md:p-6 border-b border-orange-100">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-base md:text-lg font-bold text-gray-800">Tiến hành thanh toán</h3>
        </div>
      </div>
          ...
          className={`w-full mt-4 md:mt-6 font-semibold py-3 md:py-4 px-4 md:px-6 rounded-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-3 group ${
            canCheckout
              ? checkoutStep === 'payment_failed'
                ? 'bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white hover:shadow-xl transform hover:scale-[1.02]'
        ...
        >
          {isProcessing ? (
            <>
              ...
              <span className="text-sm md:text-base">{getButtonText()}</span>
            </>
          ) : (
            <>
              {checkoutStep === 'payment_failed' ? (
                <svg
                  className="w-4 h-4 md:w-5 md:h-5 group-hover:rotate-180 transition-transform duration-200"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
