...
import { OrderItem } from '@services/order.api';
...
const BuyAgainDialog: React.FC<BuyAgainDialogProps> = ({
    isOpen,
    onClose,
    items = [],
}) => {
    ...
    const calculateOrderSummary = () => {
        ...
        const originalPrice = selectedItems.reduce((sum, product) => 
            sum + (product.currentPrice * product.selectedQuantity), 0
        );
    ...
    const validateOrder = (): string | null => {
        ...
        const invalidProduct = selectedProducts.find(p => 
            p.isSelected && (p.selectedQuantity > p.quantity || p.selectedQuantity < 1)
        );
        if (invalidProduct) {
    ...
    const canPurchase = selectedAddressId && selectedPaymentMethodId && !isProcessing && hasSelectedProducts;
                                                        ...
                                                        {product.isSelected && product.quantity > 0 && (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex items-center bg-white border border-gray-300 rounded-md overflow-hidden">
                                                                    <button
                                                                        type="button"
                                                                        onClick={() => updateSelectedQuantity(product.id, product.selectedQuantity - 1)}
