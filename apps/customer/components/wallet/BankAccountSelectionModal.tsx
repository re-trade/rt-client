...
const BankAccountSelectionModal = ({
  ...
  return (
    <dialog id="bank_account_modal" className={`modal ${bankAccountModalOpen ? 'modal-open' : ''}`}>
      <div className="bg-white text-gray-800 rounded-xl shadow-xl w-11/12 max-w-4xl p-0 overflow-hidden border border-orange-200">
        <div className="bg-gradient-to-r from-orange-50 to-orange-100 px-6 py-4 flex justify-between items-center border-b border-orange-200">
          <div className="flex items-center">
            ...
            <h3 className="text-xl font-bold text-gray-800">Chọn tài khoản ngân hàng</h3>
          </div>
          <button
        ...
        <div className="p-6">
          {loadingBankAccounts ? (
            <div className="py-12 text-center">
              ...
              <p className="mt-3 text-gray-600 font-medium">Đang tải danh sách tài khoản...</p>
            </div>
          ) : userBankAccounts.length === 0 ? (
            <div className="py-16 text-center">
              <div className="w-24 h-24 mx-auto bg-orange-100 rounded-2xl flex items-center justify-center mb-6">
                <svg
              ...
              <button
                onClick={() => setBankAccountModalOpen(false)}
                className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-6 py-3 rounded-xl transition-all duration-200 flex items-center space-x-2 mx-auto font-medium shadow-md hover:shadow-lg"
              >
                <span>Đóng</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
              {userBankAccounts.map((account) => (
                <div
                  key={account.id}
                  className="relative group border border-orange-200 rounded-xl p-4 hover:border-orange-400 hover:shadow-lg transition-all duration-300 bg-white cursor-pointer"
                  onClick={() => {
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-800 truncate">{account.bankName}</h4>
