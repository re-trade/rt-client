...
export function MultiSelectCategory({ onChange, value: initialValue = [], currentCategoryId }: Props) {
  ...
  useEffect(() => {
    if (!search.trim()) {
      ...
      const filtered = allCategories.filter((category) => category.name.toLowerCase().includes(search.toLowerCase()))
  ...
  const handleToggleCategory = (id: string) => {
    const newSelected = selected.includes(id) ? selected.filter((cid) => cid !== id) : [...selected, id]
  ...
  const getSelectedCategories = () => {
    const selectedCats = selected.map((id) => allCategories.find((c) => c.id === id)).filter(Boolean) as Category[]
  ...
  return (
    <div ref={containerRef} className="space-y-3 w-full">
              ...
              {filteredCategories.length > 0 ? (
                <div className="p-1">
                  {filteredCategories.map((category) => (
                    ...
                    >
                      <span className={cn("text-sm", selected.includes(category.id) && "font-medium text-blue-700")}>
                        {category.name}
                      </span>
                      {selected.includes(category.id) && <Check className="w-4 h-4 text-blue-600" />}
                    </div>
                  ))}
                </div>
              ) : (
      ...
      {/* Selected count */}
      {selected.length > 0 && <div className="text-xs text-gray-500">Đã chọn {selected.length} danh mục</div>}
    </div>
  )
}
