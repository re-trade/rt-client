...
import { myOrderApi, OrderCombo, OrdersResponse, OrderStatsResponse } from '@/service/myorder.api';
...
export function useMyOrder() {
  ...
  const getMyOrders = useCallback(
    async (
      ...
      try {
        ...
        const response = await myOrderApi.getMyOrders(currentPage, 10, query.toString());
  ...
  const goToPage = useCallback((page: number) => {
    setOrderState((prev) => ({
      ...prev,
      pagination: prev.pagination ? { ...prev.pagination, page: page - 1 } : null,
    }));
  }, []);
  ...
  useEffect(() => {
    const fetchOrderStats = async () => {
      try {
        const response = await myOrderApi.getOrderStats();
