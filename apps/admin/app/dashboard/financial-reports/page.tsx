...
export default function FinancialReportPage() {
  ...
  const currentData = financialData[selectedViewType.toLowerCase() as keyof typeof financialData] || []
  ...
  const revenueGrowth = previousData.length >= 2 && previousData[0] && previousData[1] && previousData[0].revenue !== 0
    ? ((previousData[1].revenue - previousData[0].revenue) / previousData[0].revenue) * 100
    : 0
  const costGrowth = previousData.length >= 2 && previousData[0] && previousData[1] && previousData[0].cost !== 0
    ? ((previousData[1].cost - previousData[0].cost) / previousData[0].cost) * 100
    : 0
  const profitGrowth = previousData.length >= 2 && previousData[0] && previousData[1] && previousData[0].profit !== 0
  ...
  const filteredTransactions = transactions?.filter((transaction) => {
    ...
    const matchesType = selectedTransactionType === "Tất cả" || transaction.type === selectedTransactionType
    const matchesStatus = selectedTransactionStatus === "Tất cả" || transaction.status === selectedTransactionStatus
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={currentData}>
                ...
                <XAxis dataKey={selectedViewType === "Ngày" ? "date" : selectedViewType === "Tuần" ? "week" : "month"} />
                <YAxis />
                <Tooltip
                  formatter={(value) => [`${value.toLocaleString('vi-VN')}đ`, "Số tiền"]}
                />
                <Legend />
                <Line
                  type="monotone"
